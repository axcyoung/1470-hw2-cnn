...
def get_data(file_path, first_class, second_class):
    ...
    desired_indices = np.nonzero((labels == first_class) | (labels == second_class))
    inputs = inputs[desired_indices]
    labels = labels[desired_indices]

    inputs = tf.reshape(inputs, (-1, 3, 32 ,32))
    inputs = tf.transpose(inputs, perm=[0,2,3,1])
    inputs = tf.cast(inputs, np.float32)
    inputs = inputs/np.float32(255.0)

    labels = np.where(labels == first_class, 0, 1)
    labels = tf.one_hot(labels, 2, dtype=tf.float32)
